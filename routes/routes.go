...
func (qh quizHandler) getMethod(w http.ResponseWriter, r *http.Request) {
	...
	letter, _ := utf8.DecodeRuneInString(qd.Answer) // TEMP(Amr Ojjeh): also temporary
	page.QuestionPage(page.QuestionParams{
		Excerpt:        qh.excerpt,
		HighlightedRef: qd.Reference,
		QuizTitle:      qh.quiz.Title,
		Prompt:         qd.Prompt,
		// TEMP(Amr Ojjeh): Only works for quiz 1. Also, URLs should be generated by functions
		// TODO(Amr Ojjeh): Automatically find the correct input method
		InputMethod: components.VowelInputMethodUnsubmitted(string(letter),
			fmt.Sprintf("/quiz/1/%d", question.Position)),
